...
if [[ ! -d /home ]]; then
    # /home doesn't exist, create it and mount
    sudo mkdir -p /home
    sudo mount -o bind "/volume1/homes" /home
elif ! grep -qs ' /home ' /proc/mounts; then
    # /home exists but is not listed as a mount in /proc/mounts, so mount it
    sudo mount -o bind "/volume1/homes" /home
fi

# Permission fix required  - do not apply recursively!
